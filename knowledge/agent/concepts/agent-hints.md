...
Agent Hint 是为某个决策提供方向、偏好、估计或风险线索但不拥有最终裁决权的辅助信号。

## 详细解释

消费者可以结合目标、事实、约束和其他证据采纳、降权或忽略 Hint；Hint 不等于 prompt，也不限定为自然语言，它可以是子目标、工具描述、风险注解、检索经验、视觉框、候选分数、请求优先级或预计输出长度。
