...
如果把 Hint 简化为 prompt，便无法解释结构化工具注解和推理调度元数据；如果把 Hint 等同于所有上下文，又失去概念的区分度。highlight：**在 Agent 系统中，什么信息应被视为 Hint，它有哪些类型，如何进入决策闭环，适合哪些场景，又该怎样与约束、反馈、记忆和策略区分？**

> 调研快照：2026-09-02。业界尚不存在统一的 Agent Hint 标准。
