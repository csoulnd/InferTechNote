...
### 8.3 跨组件一致性必须依靠状态机和补偿

注册中心、管理面数据库、文件系统和 Docker 无法组成 ACID 事务。实现应具备：
...
- 状态不确定：不要立即重复注册，进入对账；
- 数据库更新失败：利用 request ID 和注册中心回读恢复；
...
- `name + version` 唯一；
- POST 是 create、upsert 还是 compare-and-set；
- 同版本不同 digest 如何处理；
- 默认版本切换是否在注册中心事务内完成；
- 删除卡片时实例检查和记录删除是否原子；
...
最新数据库设计能够从根本上解决未注册包身份、摘要互斥、构建/注册阶段区分、只重试注册、完整 access mode 保留以及卡片单一事实源等问题。与此同时，流式上传、远端轮询超时、跨组件幂等与补偿、image-process 重启语义、路径安全、前端错误恢复和分页 bug 仍需要单独实现。
