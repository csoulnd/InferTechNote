...
迁移按照“先完成本机数据库结构和数据迁移，再切换卡片读写，最后合入描述功能和名称预检扩展”的顺序实施。注册中心已经发布的新契约作为迁移输入，不要求在数据库变更阶段同步完成所有前端功能。
...
阶段完成标准：数据库迁移所依赖的字段语义和映射规则已经明确。描述新增与修改虽然已有注册中心字段和接口支持，但不作为数据库迁移的前置条件。
...
6. 新表以增量迁移创建，不删除、不改名旧表。

阶段完成标准：新表可以独立表达未注册软件包和构建历史，但线上读写仍维持旧链路。

### 6.3 阶段 2：迁移本机构建数据并切换任务写入

1. 每条 `build_tasks` 转换为一条 `agent_build_history`。
2. 根据任务状态映射新 `phase`，保留镜像引用、摘要、错误和时间字段。
3. 按路径计算或恢复 `content_digest`；无法恢复时记录迁移异常，不伪造摘要。
4. 按路径和摘要聚合同一软件包的多次任务，识别其最新处理状态。
5. 尚未注册且本机文件存在的包生成 `local_agent_packages`；文件不存在时只保留历史。
6. 新上传与重试改写新表；观察期内可短期兼容写入 `build_tasks`。
7. 构建成功、注册失败时保留构建结果，支持只重试注册。

阶段完成标准：未注册包和构建历史不再依靠 `build_tasks - agent_registrations` 差集实时推导。卡片读取暂不切换。

### 6.4 阶段 3：最小化适配新版注册中心

本阶段只完成数据库切换必需的卡片能力，不包含描述新增和修改功能。

1. `list/get/delete/default` 改用 `name + version`。
2. `version` 为标准字段；过渡期只在客户端适配层回退读取 `framework_version`。
3. 发布请求发送 `name`、`version`、`framework` 和 `access_mode`，不再把命令写入 `framework`。
4. 上传界面在发起构建前提交完整 `access_mode`；工厂构建结果返回 `name/version` 后再注册。
5. 卡片 DTO 先投影现有功能必需的 `name/version/framework/is_default/access_mode`。
6. 前端卡片 Key、详情路由和操作参数切换为 `name + version`。
7. 修复卡片分页总数和单卡实例总数复用同一变量的问题。

### 6.5 阶段 4：迁移存量注册数据并切换读取

1. 逐条读取 `agent_registrations`，结合注册中心现有记录和构建历史确定 `name + version`。
2. 将旧 `framework` 中兼容承载的命令迁入 `access_mode[name=tui].cmd`，端口补为 `2222`。
3. 将 `installer_path` 回填为注册中心 `package_path`，补齐可确认的归档路径和运行信息；描述为空不阻塞迁移。
4. 写入注册中心后回读校验，不立即删除本地旧记录。
5. 双读对比注册中心卡片与旧拼接卡片的数量、主键和关键字段。
6. 差异收敛后，卡片列表和详情只读取注册中心。
7. 删除 `_merge_local_card()` 及卡片查询中的 `AgentRegistration` 读取。
8. 未注册列表、构建进度和历史只读取新表。
9. 实例接口失败时返回明确错误或未知状态，不投影为零。
...
5. 导出旧表备份并先改名隔离，确认运行代码不存在访问。
6. 删除旧 ORM、Service 合并逻辑、差集推导逻辑和对应兼容测试。
7. 经过约定观察期后删除旧表及旧字段兼容分支。

迁移完成标准：删除两张旧表后，卡片展示与启动、当前上架流程、未注册包重试、构建历史查询和卡片删除均正常工作。

### 6.7 阶段 6：合入描述新增与修改功能（后续）
