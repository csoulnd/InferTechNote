# Claude Code Offline Deployment Guide
...
结合 [Third-Party Agent Agent OS Requirements](./third-party-agent-agentos-requirements.md)，Claude Code 以**沙箱实例**形式运行，Gateway 通过 SSH 透传 TUI。
...
- Gateway 只做字节流透传，不解析 TUI 内容（参见 [TUI vs ACP Comparison](./tui-vs-acp-comparison.md)）。
