# NPUModelRunner（model_runner_v1.py）代码走读
...
更细的 `_prepare_inputs` 张量推导见：[Prepare inputs for model forwarding](./ModelRunner_prepare_inputs.md)。
...
阅读建议顺序：`__init__` → `execute_model` → `_prepare_inputs`（配合 [ModelRunner_prepare_inputs](./ModelRunner_prepare_inputs.md)）→ `_build_attention_metadata` → `sample_tokens` → `initialize_kv_cache` 链路。
